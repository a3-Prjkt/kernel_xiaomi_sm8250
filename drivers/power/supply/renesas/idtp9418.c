...
int idtp9220_write(struct idtp9220_device_info *di, u16 reg, u8 val)
{
	...
	if (rc < 0) {
		if ((reg == 0x3040) && (val == 0x80))
			dev_err(di->dev,
				"[idt] reset mpu and run downloader program, ignoreNAK.\n");
		else
			dev_err(di->dev,
				"[idt] idtp9220 write error: %d, reg:%x, val:%x.\n",
				rc, reg, val);
	}
...
retry_chip_fw_show:
	...
	if (di->dt_props.ra9530_enable) {
		if (!program_fw_ra9530(di, 0x0000, idt_firmware_ra9530,
				       sizeof(idt_firmware_ra9530))) {
			fw_ret = 0;
...
*/
	...
	di->dt_props.ra9530_enable =
		of_property_read_bool(node, "idt,ra9530-enable");
	if (di->dt_props.ra9530_enable) {
...
static void idtp9220_fw_download_work(struct work_struct *work)
{
	...
	if (fw_ok && crc_ok) {
		dev_info(di->dev,
			 "FW: [0]:0x%x [2]:0x%x, crc: %d so skip upgrade\n",
		...
		if (di->dt_props.ra9530_enable) {
			...
			if (!crc_rc) {
				dev_err(di->dev,
					"RA9530: crc verify failed.\n");
				di->fw_check_flag = false;
			} else {
				dev_info(di->dev,
					 "RA9530: crc verify success.\n");
				di->fw_check_flag = true;
			}
		} else {
			crc_rc = program_crc_verify(di);
	...
	if (di->dt_props.ra9530_enable) {
		...
		if ((!di->fw_check_flag) && (di->retry_num < 2)) {
			...
			dev_info(di->dev, "Retry fw_download_work, cnt:%d.\n",
				 di->retry_num);
			schedule_delayed_work(&di->fw_download_work,
...
static void idtp9220_start_to_load(struct idtp9220_device_info *di)
{
	...
	queue_delayed_work(system_power_efficient_wq, &di->rx_vout_work, msecs_to_jiffies(100));
	queue_delayed_work(system_power_efficient_wq, &di->chg_monitor_work, msecs_to_jiffies(1000));
...
static int idt_trx_get_ble_mac(struct idtp9220_device_info *di)
{
	...
	if (di->dt_props.ra9530_enable)
		rc = di->bus.read_buf(di, REG_MAC_ADDR_RA9530, mac_buf,
				      MAC_LEN);
	else
...
static void idtp9220_irq_work(struct work_struct *work)
{
	...
	if (int_val & INT_MODE_CHANGE || int_val & INT_VOUT_OFF) {
		if (di->power_good_flag) {
			...
			queue_delayed_work(system_power_efficient_wq, &di->wpc_det_work,
					      msecs_to_jiffies(0));
		}
	...
	if (int_val & INT_RXREADY) {
		...
		queue_delayed_work(system_power_efficient_wq, &di->rx_ready_check_work,
				      msecs_to_jiffies(100));
	}
...
static irqreturn_t idtp9220_hall3_irq_handler(int irq, void *dev_id)
{
	...
	if (gpio_is_valid(di->dt_props.hall3_gpio)) {
		if (gpio_get_value(di->dt_props.hall3_gpio)) {
			...
			queue_delayed_work(system_power_efficient_wq, &di->hall3_irq_work,
					      msecs_to_jiffies(0));
			return IRQ_HANDLED;
		} else {
			dev_info(di->dev,
...
static irqreturn_t idtp9220_hall4_irq_handler(int irq, void *dev_id)
{
	...
	if (gpio_is_valid(di->dt_props.hall4_gpio)) {
		if (gpio_get_value(di->dt_props.hall4_gpio)) {
			...
			queue_delayed_work(system_power_efficient_wq, &di->hall4_irq_work,
					      msecs_to_jiffies(0));
			return IRQ_HANDLED;
		} else {
			dev_info(di->dev,
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (!idt_first_flag)
		queue_delayed_work(system_power_efficient_wq, &di->idt_first_boot,
