...
static int idtp9220_set_present(struct idtp9220_device_info *di, int enable)
{
	...
	if (enable) {
		...
		queue_delayed_work(system_power_efficient_wq, &g_di->oob_set_ept_work,
				      msecs_to_jiffies(10));
		queue_delayed_work(system_power_efficient_wq, &di->fast_operate_work,
		...
		if (di->ln_psy)
			power_supply_set_property(
				di->ln_psy, POWER_SUPPLY_PROP_RESET_DIV_2_MODE,
				&val);
		/* clear TX address */
		...
		if (di->fcc_votable) {
			...
			vote(di->fcc_votable, VOICE_LIMIT_FCC_1A_VOTER, false,
			     0);
		}
		if (di->usb_psy)
...
static struct attribute *sysfs_attrs[] = {
	...
	&dev_attr_reverse_set_gpio.attr, NULL,
};
...
static void idt_voice_tx_work(struct work_struct *work)
{
	...
	if (batt_temp >= 402) {
		...
		if (di->fcc_votable) {
			effective_fcc = 1000;
			vote(di->fcc_votable, VOICE_LIMIT_FCC_1A_VOTER, true,
			     effective_fcc * 1000);
		}
	...
	if (batt_temp >= 375) {
		...
		if (di->fcc_votable) {
			effective_fcc = 2600;
			vote(di->fcc_votable, VOICE_LIMIT_FCC_VOTER, true,
			     effective_fcc * 1000);
		}
	...
	if (di->op_mode != LN8282_OPMODE_SWITCHING) {
		dev_info(
			di->dev,
			"dont rise voltage because ln8282 isn't switch mode\n");
		goto out;
	}
...
static void idtp9220_fw_download_work(struct work_struct *work)
{
	...
	if (!val.intval) {
		...
		if (fw_app_ver[3] == 0x2 && fw_app_ver[2] == 0x5 &&
		    fw_app_ver[1] == 0x1)
			di->chip_ok = 1;
		else
			di->chip_ok = 0;
