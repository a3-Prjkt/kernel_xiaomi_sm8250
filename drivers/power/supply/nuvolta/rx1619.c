...
static void rx_set_charging_param(struct rx1619_chg *chip)
{
	...
	if (g_USB_TYPE >= 9) {
		...
		if (chip->is_voice_box_tx) {
			...
			queue_delayed_work(system_power_efficient_wq, &chip->voice_tx_work,
					      msecs_to_jiffies(0));
			goto out;
		}
	...
	case 0x0f: //error cmd
		...
		if (!chip->epp &&
		    ((err_cmd == ID_CMD) || (err_cmd == AUTH_CMD) ||
		     (err_cmd == UUID_CMD) || (err_cmd == COMM_CMD) ||
...
*/
	...
	if (!g_rx1619_first_flag)
		queue_delayed_work(system_power_efficient_wq, &chip->rx_first_boot,
